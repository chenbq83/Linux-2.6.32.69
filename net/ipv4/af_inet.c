...
lookup_protocol:
	...
	list_for_each_entry_rcu(answer, &inetsw[sock->type], list) {
      ...
      // inetsw是在inet_init()时被注册的，有三种：tcp、udp、raw
   ...
   // 如果是UDP socket，ops指向的是inet_dgram_ops
   // 如果是TCP socket，ops指向的是inet_stream_ops
	sock->ops = answer->ops;
	...
	if (SOCK_RAW == sock->type) {
      // 当创建的是一个RAW socket，还会额外执行下面语句
      // 如果创建的是UDP或TCP的socket，它们是标准的套接字
      // 用[sip, sport, tip, tport]这样的四元组来查找，socket()时还缺少这些信息，因此还不能插入到内核sock表中
      // 但是如果是RAW socket，它只属于某一特定协议，查找它使用的是协议号而不是套接字的四元组
      // 因此，socket()时就通过hash()插入到内核sock表中
		inet->num = protocol;
...
static const struct net_protocol udp_protocol = {
	.handler =	udp_rcv,      // 当收到UDP报文，调用这个函数
	.err_handler =	udp_err,
...
static int __init inet_init(void)
{
	...
	if (inet_add_protocol(&icmp_protocol, IPPROTO_ICMP) < 0)      // 注册ICMP
		printk(KERN_CRIT "inet_init: Cannot add ICMP protocol\n");
	if (inet_add_protocol(&udp_protocol, IPPROTO_UDP) < 0)        // 注册UDP
		printk(KERN_CRIT "inet_init: Cannot add UDP protocol\n");
	if (inet_add_protocol(&tcp_protocol, IPPROTO_TCP) < 0)        // 注册TCP
		printk(KERN_CRIT "inet_init: Cannot add TCP protocol\n");
#ifdef CONFIG_IP_MULTICAST
	if (inet_add_protocol(&igmp_protocol, IPPROTO_IGMP) < 0)      // 注册IGMP
