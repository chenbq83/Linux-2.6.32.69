...
{
   // 要删除的条目是rt，相应的散列值是hash
   // 首先通过hash找到相应的bucket
   // 然后遍历，如果条目超时，或找到rt，则删除它
			       ...
			       u8 tos, struct net_device *dev)
{
   // 查找路由表：
   // 1. 如果是丢弃的报文，则直接drop
   // 2. 如果是不能接收或者转发的报文，则input=ip_error
   // 3. 如果是发往本机的报文，input=ip_local_deliver
   // 4. 如果是广播报文，组播报文，input=ip_local_deliver
   // 5. 如果是转发的报文，input=ip_forward
	struct fib_result res;
	struct in_device *in_dev = in_dev_get(dev);
		   ...
		   u8 tos, struct net_device *dev)
{
	...
	tos &= IPTOS_RT_MASK;
   // 先在cache中查询是否存在相应的路由表项
   // 缓存的路由表项在内核中组织成hash表的形式，
   // 因此在查询时，先算出hash值，再用该项rt_hash_table[hash].chain即可。

   // hash的参数：源IP、目的IP、进来的网卡（？）、rt_genid（是什么）
	...
	for (rth = rcu_dereference(rt_hash_table[hash].chain); rth;
	     rth = rcu_dereference(rth->u.dst.rt_next)) {
      // 遍历链表
      // 找到匹配的项（源IP、目的IP、iif、tos、mark、net）
		if (((rth->fl.fl4_dst ^ daddr) |
		     (rth->fl.fl4_src ^ saddr) |
		     (rth->fl.iif ^ iif) |
		     rth->fl.oif |
...
skip_cache:
   ...
   // 如果在缓存中没有查到匹配项，或者指定了不查找cache，
   // 则查找路由表ip_route_input_slow()
   // 进入这个函数，最终调用fib_lookup()得到查询结果fib_result
