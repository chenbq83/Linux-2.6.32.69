...
// 封装函数，在__skb_dequeue的基础上加上合法性校验或者获取锁等操作。
struct sk_buff *skb_dequeue(struct sk_buff_head *list)
{
	unsigned long flags;
	struct sk_buff *result;

	spin_lock_irqsave(&list->lock, flags);      // 获取锁
	result = __skb_dequeue(list);
	spin_unlock_irqrestore(&list->lock, flags); // 释放锁
	return result;
}
...
{
   // skb缓存池
   // 创建skbuff_head_cache高速缓存
   // 一般情况下，skb都是从这个缓存池中分配的。
   ...
   // 如果在分配skb时就知道可能被克隆，那么应该从这个高速缓存中分配空间。
   // 因为在这个高速缓存中分配skb时，会同时分配一个后备的skb，以便将来用于克隆。
   // 提高了效率。
