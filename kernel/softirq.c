...
/*
 * 下半部
 * 在中断处理过程中，不能睡眠。另外它运行的时候，会把当前中断线在所有处理器上都屏蔽（在ack中完成）
 * 更糟糕的是，如果一个处理程序是SA_INTERRUPT类型，它执行的时候会禁止所有本地中断（通过cli指令），
 * 所以，中断处理应该尽可能快的完成。
 * Linux把中断处理分为上半部和下半部。
 * 上半部由中断处理程序完成，它通常完成一些和硬件相关的操作，比如对中断的到达的确认。有时候它还会从
 * 硬件拷贝数据，这些工作对时间非常敏感，只能靠中断处理程序自己完成。而把其他工作放到下半部完成。
 * 下半部的执行不需要一个确切的时间，它会在稍后系统不太繁忙时执行。下半部执行的关键在于运行的时候
 * 允许响应所有的中断。最早，Linux用“bottom half”实现下半部，简称BH，但是即使属于不同的处理器，也不允许
 * 任何两个bottom half同时执行，这种机制简单，但是性能有限。不久，又引入了任务队列（task queue）机制
 * 来实现下半部，但该机制扔不够灵活，没法代替整个BH接口。
 * 从2.3开始，内核引入软中断（softirq）和tasklet，并完全取代了BH。2.5中，BH最终舍去。在2.6中，内核
 * 拥有三种机制实现下半部：软中断、tasklet和工作队列。
 * Tasklet是基于软中断实现的。软中断可以在多个CPU上同时执行，即使它们是同一类型的，所以，软中断
 * 处理程序必须是可重入的，或者显式地用自旋锁保护相应的数据结构。而相同的tasklet不能同时在多个CPU上
 * 执行，所以tasklet不必是可重入的；但是，不同类型的tasklet可以在多个CPU上同时执行。
 * 一般来说，tasklet比较常用，它可以处理绝大部分的问题；而软中断用的比较少，但是对于时间要求比较高的
 * 地方，比如网络子系统，常用软中断处理下半部工作。
 */
...
{
   // softirq_vec是一个全局变量，系统中每个CPU所看到的是同一个数组。
   // 但是，每个CPU各有其自己的“软中断控制/状态”结构，这些数据结构形成一个以CPU编号为下标
   // 的数组irq_stat[]
