...
static int b44_rx(struct b44 *bp, int budget)
{
   ...
   // bp->rx_prod 当前缓冲区的最后一个缓冲号
	...
	received = 0;
   // prod读取B44_DMARX_STAT寄存器，存储网卡当前处理到的缓冲区号
   // cons存储处理器处理到的缓冲区号
	prod  = br32(bp, B44_DMARX_STAT) & DMARX_STAT_CDMASK;
	prod /= sizeof(struct dma_desc);
	cons = bp->rx_cons;

   // 网卡读取B44_DMARX_ADDR与B44_DMARX_STAT寄存器，
   // 得到下一个处理的struct dma_desc，
   // 然后根据dma_desc中的addr找到报文缓冲区，
   // 通过DMA处理器将网卡收到的报文拷贝的addr地址处。
   // 这个过程CPU是不参与的！！！
	while (cons != prod && budget > 0) {
      ...
      // 从这里可以看出，B440X网卡驱动不会检查CRC校验。
		...
		if (!bp->force_copybreak && len > RX_COPY_THRESHOLD) {
         ...
         // 3. 将缓冲区号rx_prod控制权交给网卡
         // 简单地说，就是将cons号缓冲区交由CPU处理，而rx_prod号缓冲区代替给网卡使用
			b44_recycle_rx(bp, cons, bp->rx_prod);
         ...
         // 在报文头部保留了两个字节。
         // 链路层报头是14字节，正常IP报文会从14字节开始，这样就不是4字节对齐的了。
         // 所以在头部保留2个字节，使IP报文从16字节开始
			skb_reserve(copy_skb, 2);
			skb_put(copy_skb, len);

         // CPU将报文从skb拷贝到copy_skb中，跳过了网卡报文的额外信息，
         // 因为这部分信息在上层协议栈是没用的，所以去掉。
   ...
   // 如此循环直到cons == prod，此时网卡收到的报文都已被CPU处理，更新变量
	bp->rx_cons = cons;
...
{
   // 当有数据包收发或者发生错误，会产生硬件中断，调用netdev_priv()
   ...
   // 读出当前的中断状态字和中断屏蔽字
	...
	if (istat) {
		...
		if (napi_schedule_prep(&bp->napi)) {
			 ...
			 */
         // 设置NAPI为SCHED状态，记录当前的中断状态关闭中断，执行调度
			bp->istat = istat;
			__b44_disable_ints(bp);
...
static int b44_alloc_consistent(struct b44 *bp, gfp_t gfp)
{
   ...
   // rx_ring是DMA映射的虚拟地址
   // rx_ring_dma是DMA映射的总线地址，这个地址会写入B44_DMARX_ADDR寄存器（网卡的寄存器），作为环形缓冲区的基地址
				  ...
				  const struct ssb_device_id *ent)
{
   ...
   // b44_poll赋给napi_poll
   // 所以在NET_RX_SOFTIRQ软中断处理函数net_rx_action()中执行的是b44_poll()
